...
{	
	...
	gStateData.process_var = 20.0;   //过程噪声
	gStateData.acc_var = 2.0;        //测量噪声
	gStateData.mag_var = 2.0;        //测量噪声
...
void EKF_Predict( AHRS_state_data* estimated_states, SensorData* sensor_data )
{
	...
	quat pqr_quat;
	quat temp_quat;
	fMatrix A,Atranspose,temp1,temp2;  //A是系统参数，卡尔曼滤波前半系统公式：X(k)=AX(k-1)+BU(k)+W(k),W(k)即Q，过程噪声
	//乘以1.5是什么意思，interval_us是否准确？也就是systick end位置对吗《==》
	T = (float)(0.000001)*(float)interval_us ;//* 1.5;
	// Copy body frame angular rates to local variables for convenience
	// 0.2686976  = 16.4/((2000/32768)*10^3)
//  	p = (gStateData.gyro_x - gSensorData.gyro_zero_norm[1] ) * 0.2686976;
//  	q = (gStateData.gyro_y - gSensorData.gyro_zero_norm[0])  * 0.2686976;
//  	r = (gStateData.gyro_z - gSensorData.gyro_zero_norm[2])  * 0.2686976;
   	p = gStateData.gyro_x;    
   	q = gStateData.gyro_y;
   	r = gStateData.gyro_z;
	...
	// Compute linearized state transition matrix  //计算线性的状态转换矩阵  	A是状态转换矩阵	R是过程噪声矩阵
	/*		[       1, -(T*p)/2, -(T*q)/2, -(T*r)/2]	 //	Sigma是协方差
			[ (T*p)/2,        1,  (T*r)/2, -(T*q)/2]
			[ (T*q)/2, -(T*r)/2,        1,  (T*p)/2]
			[ (T*r)/2,  (T*q)/2, -(T*p)/2,        1]
	*/		
...
void EKF_Update( AHRS_state_data* estimated_states, SensorData* sensor_data )
{
	 // QUATERNION CODE
	 {
		  ...
		  if( gSensorData.new_acc_data )//&& (gConfig.r[UM6_MISC_CONFIG] & UM6_ACCEL_UPDATE_ENABLED) )
		  {
				float sensor_norm = sqrt(gStateData.acc_x*gStateData.acc_x + gStateData.acc_y*gStateData.acc_y + gStateData.acc_z*gStateData.acc_z);
				fMatrix C;
				...
				if( (sensor_norm - 1) < 0.1 && (sensor_norm - 1) > -0.1 )
				{
					 ...
					 ax_ref = gStateData.acc_ref_x;
					 ay_ref = gStateData.acc_ref_y;
					 az_ref = gStateData.acc_ref_z;
					 ...
					 */
					 ...
					 // Do correction	测得的加速度值		计算的加计	加速度协方差
					 EKF_Correction( &C, gStateData.acc_x, ax_hat, gStateData.acc_var, &gStateData, ACCEL_UPDATE );
					 ...
					 // Do correction
					 EKF_Correction( &C, gStateData.acc_y, ay_hat, gStateData.acc_var, &gStateData, ACCEL_UPDATE );
					 ...
					 // Do correction
					 EKF_Correction( &C, gStateData.acc_z, az_hat, gStateData.acc_var, &gStateData, ACCEL_UPDATE );
