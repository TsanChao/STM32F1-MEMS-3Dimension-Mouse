...
typedef struct _ENDPOINT_INFO
{
  ...
  */
  ...
  u16  Usb_wOffset;
  u16  PacketSize;
  u8   *(*CopyData)(u16 Length);	           //	 在set0_process中用 CopyRoutine	 赋值
}
ENDPOINT_INFO;
...
{
  u8 USBbmRequestType;       /* bmRequestType */  //标示   HID  设备类型值 
  u8 USBbRequest;            /* bRequest */	    //请求码   GetReport,Getprotol等
  u16_u8 USBwValues;         /* wValue */  //    报告的类型 0x01输入报告，0x02输出报告，0x03特征报告
  u16_u8 USBwIndexs;         /* wIndex */	     //指明支持此请求的接口号码
  u16_u8 USBwLengths;        /* wLength */	    //报告长度
...
/* Exported constants --------------------------------------------------------*/
#define Type_Recipient (pInformation->USBbmRequestType & (REQUEST_TYPE | RECIPIENT))  // 0x80&( 0x60 | 0x1F )
